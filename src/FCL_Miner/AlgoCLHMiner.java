...
public class AlgoCLHMiner {
	...
	int countTrans;
	int countmax = (int) (1112949 * 1);
	int timeProject = 0;
	boolean isNew;

	public void runAlgorithm(double minUtil, String inputPath, String outputPath, String TaxonomyPath, boolean isNew)
			throws IOException {

		this.minUtil = minUtil;
		this.isNew = isNew;
		candidate = 0;// Tập ứng viên
		maxMemory = 0;
		...
		countHUI = 0; // Đếm số lượng CLHUI -
		countTrans = 0; // Số lượng transaction
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null && countTrans <= countmax) {
		...
		List<UtilityList> listOfUtilityLists = new ArrayList<UtilityList>();// tid , iutil , rutil
		...
		for (Integer item : mapItemToTWU.keySet()) {
			...
			if (mapItemToTWU.get(item) >= minUtil)// CẮT TỈA TRÊN TRANSACTION UTIL
			{
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null && countTrans <= countmax) {
				...
				double TU = remainingUtility = Double.parseDouble(split[1]);
				// Create a list to store items
				List<Pair> revisedTransaction = new ArrayList<Pair>(); // lấy tất cả các item thỏa twu trong từng trans
				// for each item
...
//				System.out.println("  #UTIL: " + X.sumIutils);
			...
			for (int j = i + 1; j < ULs.size(); j++) {
				...
				if (compareItems(Y.item, X.item) > 0) {
					if (!CheckParent(Y.item, X.item) && Y.item != X.item) {
						UtilityList exULBuild = isNew ? construct_new(pUL, X, Y, prefix, minUtil)
								: construct_old(pUL, X, Y, prefix);
						if (isNew)
							if (exULBuild != null && exULBuild.GWU > minUtil) {
								exULs.add(exULBuild);
							}
						if (!isNew)
	...
	private UtilityList construct_new(UtilityList P, UtilityList px, UtilityList py, int[] prefix, double minU) {
		...
		double totalUtility = px.sumIutils + px.sumRutils;// X
		// ================================================
		double Y = px.sumIutils;
		int indexX = 0;
		int indexY = 0;
		int xMax = px.elements.size();
		int yMax = py.elements.size();
		while (indexX < xMax && indexY < yMax) {
			Element ex = px.elements.get(indexX);
			Element ey = py.elements.get(indexY);

			if (ex.tid == ey.tid) {
				double Pfutil = 0;
				if (P != null) {
					// Pfutil = findElementWithTID(P, ex.tid).iutils;
					// Upgrade
					...
					// double util = ex.iutils + ey.iutils - Pfutil;
					// double rutil = ex.rutils > ey.rutils ? ey.rutils : ex.rutils;
					// Upgrade
				...
				Y = Y + (ex.iutils - Pfutil);
				if (ex.rutils == 0 && Y < minU) {
					return null;
				}
				indexX++;
				indexY++;
	...
	private UtilityList construct_old(UtilityList P, UtilityList px, UtilityList py, int[] prefix) {
		UtilityList pxyUL = new UtilityList(py.item);
	...
	public String printStats() throws IOException {
		String valsString = "";
		System.out.println("=============  CLH-Miner =============");
		System.out.println(" |D|              : " + countTrans);
		...
		valsString += "\n=============  CLH-Miner =============" + "\n |D|              : " + countTrans
				...
				+ maxMemory + " MB" + "\nCLHUIs/Candidates: " + countHUI + "/" + candidate
				+ "\n======================================";
		return valsString;
	}
